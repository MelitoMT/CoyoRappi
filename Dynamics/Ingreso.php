...
if (isset($_SESSION['usuario']) && (isset($_SESSION['contrasenia'])))
{
  echo "La sesion de ".$_SESSION['usuario']."  esta activa";
                ...
                echo "<input type='submit' name='close' value='Cerrar Sesion'>";
                echo "</form>";
}
else
{
                          if (isset($_POST['alumno']) && $_POST['alumno']!=""
                            &&(isset($_POST['numCuenta']) && $_POST['numCuenta']!=""))
                            {
                              ...
                              $contra=password_hash($alumno, password_BCRYPT);
...
echo "<br><br>";
echo "<form action='Ingreso.php' method='POST'>";
echo "<input type='submit' name= 'Regresar' value='Regresar al menu principal'>";
echo "</form>";

if(isset($_POST["Regresar"]))
{
  /*Si el boton de cerrar sesion fue presionado
  se llama a la funcion homónima y le envia a la pestaña principal
  */
  cerrarSesion();
  header("Location:../templates/CoyoRappi.hmtl");
